...
c       wwn=938.926d0
       ...
       do i=1,jnum
          read(888,*) xkf(i), xmu_05(i), xmu_10(i), xmu_15(i), 
     1                xmu_20(i), xmu_25(i), xmu_30(i), xmu_35(i)
       end do
...
c       k=1
     ...
     1       ws(j),0.d0,0.d0,wwn,1,6,0)
          ...
          go to (0005,0010,0015,0020,0025,0030,0035) k
            
 0005        df=dfd(spex,T(k),xmu_05(j))
             go to 151
 0010        df=dfd(spex,T(k),xmu_10(j)) 
             go to 151      
 0015        df=dfd(spex,T(k),xmu_15(j))
             go to 151
 0020        df=dfd(spex,T(k),xmu_20(j))
             go to 151
 0025        df=dfd(spex,T(k),xmu_25(j))
             go to 151
 0030        df=dfd(spex,T(k),xmu_30(j))
             go to 151
 0035        df=dfd(spex,T(k),xmu_35(j))
